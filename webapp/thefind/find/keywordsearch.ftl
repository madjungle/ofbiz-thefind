    ...
    <table cellspacing="0" class="basic-table">
        <tr>
          ...
          <td align="right">
            <b>
            <#if 0 < viewIndex?int>
              <a href="<@ofbizUrl>keywordsearch/~VIEW_INDEX=${viewIndex-1}/~VIEW_SIZE=${viewSize}/~clearSearch=N/~PAGING=${paging}/~noConditionFind=${noConditionFind}/~productStoreId=${parameters.productStoreId?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonPrevious}</a> |
            </#if>
            <#if 0 < listSize?int>
              ${lowIndex+1} - ${highIndex} ${uiLabelMap.CommonOf} ${listSize}
            </#if>
            <#if highIndex?int < listSize?int>
              | <a href="<@ofbizUrl>keywordsearch/~VIEW_INDEX=${viewIndex+1}/~VIEW_SIZE=${viewSize}/~clearSearch=N/~PAGING=${paging}/~noConditionFind=${noConditionFind}/~productStoreId=${parameters.productStoreId?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonNext}</a>
            </#if>
            <#if paging == "Y">
              <a href="<@ofbizUrl>keywordsearch/~VIEW_INDEX=0/~VIEW_SIZE=99999/~clearSearch=N/~PAGING=N/~noConditionFind=${noConditionFind}/~productStoreId=${parameters.productStoreId?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonPagingOff}</a>
            <#else>
              <a href="<@ofbizUrl>keywordsearch/~VIEW_INDEX=0/~VIEW_SIZE=${previousViewSize}/~clearSearch=N/~PAGING=Y/~noConditionFind=${noConditionFind}/~productStoreId=${parameters.productStoreId?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonPagingOn}</a>
            </#if>
            </b>
          </td>
        </tr>
